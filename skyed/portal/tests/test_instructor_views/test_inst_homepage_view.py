...
class TestInstHomepageView(APITestCase):
	def setUp(self):
		self.client = Client()
	...
	def test_instructor_homepage_GET(self):
		self.client.login(email="test@instructor.com", password="123kdcow")

		response = self.client.get(reverse('portal:instructors_homepage'))

		self.assertEquals(response.status_code, 200)
		self.assertTemplateUsed(response, 'portal/instructor/homepage.html')

	def test_instructor_homepage_GET_nouser(self):
		response = self.client.get(reverse('portal:instructors_homepage'))

		self.assertRedirects(response, '/portal/login?next=/portal/instructor/homepage/', status_code=302, target_status_code=301, msg_prefix='', fetch_redirect_response=True)

	def test_instructor_homepage_GET_notinstructor(self):
