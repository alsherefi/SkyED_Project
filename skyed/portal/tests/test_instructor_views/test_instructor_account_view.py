...
class TestInstAccountView(APITestCase):
	def setUp(self):
		self.client = Client()
	...
	def test_instructor_account_GET(self):
		self.client.login(email="test@instructor.com", password="123kdcow")

		response = self.client.get(reverse('portal:instructor_account'))

		self.assertEquals(response.status_code, 200)
		self.assertTemplateUsed(response, 'portal/instructor/account.html')

	def test_instructor_account_GET_nouser(self):
		response = self.client.get(reverse('portal:instructor_account'))

		self.assertRedirects(response, '/portal/login?next=/portal/instructor/account/', status_code=302, target_status_code=301, msg_prefix='', fetch_redirect_response=True)

	def test_instructor_account_GET_notinstructor(self):
