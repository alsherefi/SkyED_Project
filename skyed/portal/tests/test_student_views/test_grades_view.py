...
class TestGradesView(APITestCase):
	def setUp(self):
		self.client = Client()
	...
	def test_grades_GET_nouser(self):
		response = self.client.get(reverse('portal:grades_view', args=['1']))

		self.assertEquals(response.status_code, 302)

	def test_grades_GET_notstudent(self):
	...
	def test_grades_GET_default_semester_no_sections(self):
		self.client.login(email="test@student.com", password="123kdcow")
		response = self.client.get(reverse('portal:grades_view', args=['0']))

		self.assertEquals(response.status_code, 200)
		self.assertTemplateUsed(response, 'portal/student/grades.html')


	def test_grades_GET_default_semester_graded(self):
		self.client.login(email="test@student.com", password="123kdcow")
		course = Course.objects.create(course_id="0418141", course_name="Programming 1", credits=4, department=self.department)
		section = Section.objects.create(course=course, snumber='01A', semester=self.semester, days='135', start_time=datetime.now(), end_time=datetime.now() + timedelta(hours=1))

		StudentSection.objects.create(student=self.student, section=section, grade='B')

		response = self.client.get(reverse('portal:grades_view', args=[str(self.semester.id)]))

		self.assertEquals(response.status_code, 200)
		self.assertEquals(response.context['studentsec'][0].grade, 'B')
		self.assertTemplateUsed(response, 'portal/student/grades.html')

	def test_grades_GET_default_semester_not_graded(self):
		self.client.login(email="test@student.com", password="123kdcow")
		course = Course.objects.create(course_id="0418141", course_name="Programming 1", credits=4, department=self.department)
		section = Section.objects.create(course=course, snumber='01A', semester=self.semester, days='135', start_time=datetime.now(), end_time=datetime.now() + timedelta(hours=1))
		...
		self.assertEquals(response.status_code, 200)
		self.assertEquals(response.context['studentsec'][0].grade, None)
		self.assertTemplateUsed(response, 'portal/student/grades.html')

	def test_grades_GET_other_semester(self):
		self.client.login(email="test@student.com", password="123kdcow")
		...
		course = Course.objects.create(course_id="0418141", course_name="Programming 1", credits=4, department=self.department)
		section = Section.objects.create(course=course, snumber='01A', semester=other_semester, days='135', start_time=datetime.now(), end_time=datetime.now() + timedelta(hours=1))

		StudentSection.objects.create(student=self.student, section=section)

		response = self.client.get(reverse('portal:grades_view', args=[str(other_semester.id)]))

		self.assertEquals(response.status_code, 200)
		self.assertEquals(response.context['studentsec'][0].grade, None)
		self.assertTemplateUsed(response, 'portal/student/grades.html')

	def test_grades_GET_no_semester(self):
		self.client.login(email="test@student.com", password="123kdcow")
