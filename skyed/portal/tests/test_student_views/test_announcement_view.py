...
class TestAnnouncementView(APITestCase):
	def setUp(self):
		self.client = Client()
	...
	def test_announcements_GET_nouser(self):
		response = self.client.get(reverse('portal:announcement_view'))

		self.assertRedirects(response, '/portal/login?next=/portal/student/announcement/', status_code=302, target_status_code=301, msg_prefix='', fetch_redirect_response=True)

	def test_announcements_GET_notstudent(self):
	...
	def test_announcements_GET(self):
		...
		self.assertTemplateUsed(response, 'portal/student/announcement.html')

	def test_announcements_GET_no_semester(self):
		self.client.login(email="test@student.com", password="123kdcow")
